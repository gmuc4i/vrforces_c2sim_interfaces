...
int C2SIMOrderHandler::getRoutePoints(double x[], double y[], double z[])
{
	...
	if (elevationPointCount < pointCount)pointCount = elevationPointCount;
	...
	AttributeList& attributes)
{
	...
	if (!foundRootTag) {
		if (strncmp(startTag, orderRootTag, MAXCHARLENGTH) == 0)
			foundRootTag = true;
	}
...
void C2SIMOrderHandler::endElement(const XMLCh* const name)
{
	...
	if (!foundFinalTag) {
		if (strncmp(endTag, orderRootTag, MAXCHARLENGTH) == 0)
			foundFinalTag = true;
	}
...
void C2SIMOrderHandler::characters(
	...
	if (!foundRootTag) {
		if (strncmp(latestTag, orderRootTag, MAXCHARLENGTH) == 0) {
			foundRootTag = true;
			return;
		}
		else {
			std::cout << "order root tag " << orderRootTag << " not found in document\n";
			missingRootTag = true;
		}
	}
	...
	if (!foundTaskersIntent && strncmp(latestTag, taskersIntentTag, MAXCHARLENGTH) == 0) {
	...
	// look for UnitId
	if (!foundUnitID && strncmp(latestTag, unitIdTag, MAXCHARLENGTH) == 0) {
		foundUnitID = true;
	...
	// look for dateTime
	if (!foundDateTime && strncmp(latestTag, dateTimeTag, MAXCHARLENGTH) == 0) {
		foundDateTime = true;
	...
	if (strncmp(latestTag, elevationTag, MAXCHARLENGTH) == 0) {
		if (elevationPointCount >= MAXPOINTS) {
			std::cout << "too many longitudes; deleted " << elevationTag << "\n";
			return;
		}
